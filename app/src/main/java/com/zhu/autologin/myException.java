...
public class myException implements Thread.UncaughtExceptionHandler {
    ...
    // Log文件路径
    private static final String LOG_FILE_DIR = "/storage/emulated/0/autologin/Exception/";
    // log文件的后缀名
    ...
    private myException(Context cxt) {
        // 获取系统默认的异常处理器
    ...
    public synchronized static myException create(Context cxt) {
        if (instance == null) {
            instance = new myException(cxt);
        }
        return instance;
    }
    ...
    private void saveToSDCard(Throwable ex) throws Exception {
        File file = new File(LOG_FILE_DIR, getDataTime("yyyy-MM-dd-HH-mm-ss") + FILE_NAME);
        if (file.exists()) ;
        else {
            file.createNewFile();
